...
#define CUR (*this)
  ...
  vector<int> v;
  BigInt() {}
  BigInt(const long long &val) { CUR = val; }
  ...
  bool operator>(const BigInt &a) const { return a < CUR; }
  bool operator==(const BigInt &a) const { return (!(CUR < a) && !(a < CUR)); }
  bool operator<=(const BigInt &a) const { return ((CUR < a) || !(a < CUR)); }
  BigInt operator+(const BigInt &a) const {
    BigInt res = CUR;
    int idx = 0, carry = 0;
    while (idx < a.size() || carry) {
      58 if (idx < a.size()) carry += a.v[idx];
  ...
  friend ostream &operator<<(ostream &out, const BigInt &a) {
    ...
    for (int i = (int)a.v.size() - 2; i >= 0; i--) out << a.v[i];
    return out;
  }
#undef CUR
};
