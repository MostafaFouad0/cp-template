...
int N, start, sink, K;                       // find the k-th shortest
path from start to end int adj[SIZE][SIZE];  // this can be adjList instead of adjMatrix
    // Returns -1 of no k-th shortest path exist between start and end int
getKthShortestPath() {
  multiset<edge> pq;  // first is cost and second is
  node edge e = {-1, start, 0};
  pq.insert(e);
  vector<int> reached[N];
  // reached[i][j] is the cost of the j-th shortest
  path from start to i while (!pq.empty()) {
    edge e = *pq.begin();
    pq.erase(pq.begin());
